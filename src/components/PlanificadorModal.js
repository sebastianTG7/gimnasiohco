...
const PlanificadorModal = ({ isOpen, onClose, schedule, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-40 flex justify-center items-center p-4">
      <div className="bg-gray-800 rounded-xl shadow-2xl p-6 md:p-8 w-full max-w-3xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center mb-6">
          <h2 className="bebas-font text-4xl text-[#379AA5] tracking-wider">Planificar Semana</h2>
                  ...
                  {muscleGroups.map(group => {
                    ...
                    return (
                      <div key={`${day}-${group}`} onClick={() => handleCheckboxChange(day, group)} className="cursor-pointer flex items-center">
                        <div className={`w-5 h-5 flex justify-center items-center border-2 ${isChecked ? 'border-cyan-500 bg-cyan-500' : 'border-gray-500'} rounded-md mr-2 transition-all`}>
